...
class MainActivity : AppCompatActivity() {
    ...
    fun onUpdateBtn(view: View){
        ...
        val switchArmed = findViewById<Switch>(R.id.switch1);
        ...
        cmdBuilder.append("{\"commands\": {\"alarmMode\": ")
            .append(if(switchArmed.isChecked) "\"ARMED\"" else "\"OFF\"")
            .append("},\"sensors\": [")
    ...
    private fun getStatusSuccessCallback(it: RestResponse){
        ...
        if(item != null) {
            this@MainActivity.runOnUiThread(java.lang.Runnable {
                ...
                switchArmed.isEnabled = true
                switchArmed.isChecked = try{
                    //item.getJSONObject("armed").getBoolean("BOOL")
                    "ARMED" == item.getJSONObject("alarmMode").getString("S")
                }catch (error: JSONException) {
                    Log.e("HomeAlarmApp", "JSON Exception", error);
                    false;
                }
