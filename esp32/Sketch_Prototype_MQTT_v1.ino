  ...
  if(cm.isWiFiConnected() && cm.isAwsConnected()){
    ...
    if(currentMode == SYNC){
      ...
      if(alarmedSensors.size() > 0){
        ...
        serializeAndSend(alarmedSensors, true,  "ARMED", "NOTIFY");
        currentStatus = ALARM;
      ...
      // manage ARMED mode and NON Alarm Status (NORMAL or CONFIG_UPDATE)
        ...
        if(((nowTime - lastMessageTime) > MAX_SILENCE_TIME) || (currentStatus != NORMAL)){
          serializeAndSend(bc.getAllSensors(), false, "ARMED", "NOTIFY");
      ...
      bc.refresh();
      serializeAndSend(bc.getAllSensors(), false, "MONITOR", "NOTIFY");
